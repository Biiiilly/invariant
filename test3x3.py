...
def tidy(text):
    ...
    for line in lines:
        ...
        if line_stripped.startswith("//"):
            continue
        if "SINGULAR" in line_stripped or "Development" in line_stripped:
            continue
        if "Universitaet" in line_stripped:
            continue
        if "Auf Wiedersehen." in line_stripped:
            continue

        polynomial_lines.append(line_stripped)

    poly_string = " ".join(polynomial_lines)

    raw_polys = poly_string.split(",")

    polys = [p.strip() for p in raw_polys if p.strip()]

    return polys


def generate_invariants(n, deg):
